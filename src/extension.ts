...
export function activate(context: ExtensionContext) {
    const importCommand = commands.registerCommand(
        ...
        async () => {
            ...
            workspace.workspaceFolders?.map((folder) => {
                if (wsPath == undefined) wsPath = folder.uri;
            });
    ...
    const exportCommand = commands.registerCommand(
        'CodeViz.export',
        async () => {
            const folderPath = await window.showInputBox({
    ...
    const showCommand = commands.registerCommand(
        ...
        (targetDir: Uri | undefined) => {
            ...
            commands
                .executeCommand(
                    'extension.vscode-counter.countInDirectory',
                    targetDir,
                )
                .then(() => {
                    setTimeout(() => {
                        CCVizzuPanel.render(context.extensionUri).then(() => {
                            ...
                            if (targetDir == undefined) targetDir = wsPath;
                            source = new VSCCDataSource(wsPath);
                            if (source.data != undefined) {
                                let data = new VSCCDataPrep();
                                data.makeDataTable(source.data);
                                CCVizzuPanel.refresh(
                                    data.getDataTable(),
                                    new Summary(source, data),
                                );
                            } else
                                window.showErrorMessage(
                                    "'Code Counter' data is not awailable!",
                                );
                        });
                    }, 1000);
                });
